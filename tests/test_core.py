...
class HermesLabSmokeTests(unittest.TestCase):
    ...
    def test_openai_mutation_adapter_applies_repo_relative_command(self) -> None:
        workspace = self.make_git_workspace(initial_value=1, mutated_value=3)
        server = FakeOpenAIServer(
            [
                self.fake_openai_response(
                    {
                        "outcome": "apply",
                        "summary": "Raise the tracked value.",
                        "rationale": "The validator directly reads state.txt, so updating it improves the score.",
                        "files_considered": ["state.txt", "validate.py"],
                        "tests_or_followups": ["Run the validator."],
                        "edits": [
                            {
                                "path": "state.txt",
                                "action": "write",
        ...
        mutation_command = (
            ...
            "--model fake-codex "
            f"--base-url {shlex.quote(server.base_url)} "
            "--instruction 'Raise the value if the validator is direct and cheap.'"
        )
        create_experiment(
            self.paths,
            self.make_code_spec(
        ...
        messages = run_once(self.paths, max_runs=1)
        self.assertEqual(len(messages), 1)
        self.assertEqual((workspace / "state.txt").read_text().strip(), "3")

        run_dir = next((self.paths.experiments / "code-openai" / "runs").iterdir())
        self.assertTrue((run_dir / "artifacts" / "openai" / "request.json").exists())
        self.assertTrue((run_dir / "artifacts" / "openai" / "response.json").exists())
        self.assertTrue((run_dir / "artifacts" / "openai" / "plan.json").exists())

        self.assertEqual(len(server.requests), 1)
        request_payload = server.requests[0]["payload"]
        self.assertIsInstance(request_payload, dict)
        self.assertEqual(request_payload["store"], False)
        self.assertEqual(request_payload["text"]["format"]["type"], "json_schema")
        self.assertEqual(request_payload["model"], "fake-codex")
        self.assertIn("Mutable Paths", request_payload["input"][1]["content"])

    def test_openai_mutation_adapter_rejects_out_of_bounds_edit(self) -> None:
        workspace = Path(self._tmpdir.name) / "adapter-workspace"
        workspace.mkdir()
        (workspace / "state.txt").write_text("1\n")
        artifacts_dir = Path(self._tmpdir.name) / "adapter-artifacts"
        artifacts_dir.mkdir()
                ...
                self.fake_openai_response(
                    {
                        "outcome": "apply",
                        "summary": "Unsafe edit.",
                        "rationale": "This intentionally targets a read-only path.",
                        "files_considered": ["state.txt"],
                        "tests_or_followups": [],
                        "edits": [
                            {
                                "path": "README.md",
        ...
        env.update(
            {
                ...
                "LAB_EFFECTIVE_WORKSPACE_ROOT": str(workspace),
                "LAB_RUN_ARTIFACTS_DIR": str(artifacts_dir),
                "LAB_MUTABLE_PATHS": "state.txt",
                "LAB_READ_ONLY_PATHS": "",
                "LAB_PRIMARY_METRIC": "validation_score",
                "LAB_METRIC_DIRECTION": "maximize",
                "LAB_ACCEPTANCE_RULE": "Keep only strict improvements",
            }
        )
        ...
        completed = subprocess.run(
            [
                "python3",
                str(REPO_ROOT / "scripts" / "openai_mutation_adapter.py"),
                "--model",
                "fake-codex",
                "--base-url",
                server.base_url,
        ...
        self.assertIn("Edit escapes mutable surface", completed.stderr)
        self.assertEqual((workspace / "state.txt").read_text().strip(), "1")

    def test_claude_mutation_adapter_applies_repo_relative_command(self) -> None:
        workspace = self.make_git_workspace(initial_value=1, mutated_value=4)
        server = FakeAnthropicServer(
            [
                self.fake_anthropic_response(
                    {
                        "outcome": "apply",
                        "summary": "Raise the tracked value.",
                        "rationale": "The validator directly measures state.txt, so raising it improves the score.",
                        "files_considered": ["state.txt", "validate.py"],
                        "tests_or_followups": ["Run the validator."],
                        "edits": [
                            {
        ...
        os.environ["ANTHROPIC_BASE_URL"] = server.base_url
        ...
        mutation_command = (
            ...
            "--effort medium "
            f"--base-url {shlex.quote(server.base_url)} "
            "--instruction 'Keep the patch minimal and avoid overengineering.'"
        )
        create_experiment(
            self.paths,
            self.make_code_spec(
        ...
        messages = run_once(self.paths, max_runs=1)
        self.assertEqual(len(messages), 1)
        self.assertEqual((workspace / "state.txt").read_text().strip(), "4")

        run_dir = next((self.paths.experiments / "code-claude" / "runs").iterdir())
        self.assertTrue((run_dir / "artifacts" / "claude" / "request.json").exists())
        self.assertTrue((run_dir / "artifacts" / "claude" / "response.json").exists())
        self.assertTrue((run_dir / "artifacts" / "claude" / "plan.json").exists())

        self.assertEqual(len(server.requests), 1)
        request_payload = server.requests[0]["payload"]
        self.assertIsInstance(request_payload, dict)
        self.assertEqual(request_payload["model"], "claude-test")
        self.assertEqual(request_payload["output_config"]["effort"], "medium")
        self.assertEqual(request_payload["output_config"]["format"]["type"], "json_schema")

    def test_claude_mutation_adapter_rejects_out_of_bounds_edit(self) -> None:
        workspace = Path(self._tmpdir.name) / "claude-adapter-workspace"
        workspace.mkdir()
        (workspace / "state.txt").write_text("1\n")
        artifacts_dir = Path(self._tmpdir.name) / "claude-adapter-artifacts"
        artifacts_dir.mkdir()
                ...
                self.fake_anthropic_response(
                    {
                        "outcome": "apply",
                        "summary": "Unsafe edit.",
                        "rationale": "This intentionally targets a non-mutable path.",
                        "files_considered": ["state.txt"],
                        "tests_or_followups": [],
                        "edits": [
                            {
        ...
        completed = subprocess.run(
            [
                "python3",
                str(REPO_ROOT / "scripts" / "claude_mutation_adapter.py"),
                "--model",
                "claude-test",
                "--base-url",
                server.base_url,
    ...
    def test_local_agent_router_uses_openai_provider_fields(self) -> None:
        workspace = self.make_git_workspace(initial_value=1, mutated_value=8)
        (workspace / ".mutation.md").write_text("Raise the score if it is safe.\n")
                ...
                self.fake_openai_response(
                    {
                        "outcome": "apply",
                        "summary": "Raise the tracked value.",
                        "rationale": "The validator directly reads state.txt.",
                        "files_considered": ["state.txt", "validate.py"],
                        "tests_or_followups": ["Run the validator."],
                        "edits": [
                            {
        ...
        messages = run_once(self.paths, max_runs=1)
        self.assertEqual(len(messages), 1)
        self.assertEqual((workspace / "state.txt").read_text().strip(), "8")

        run_dir = next((self.paths.experiments / "local-openai" / "runs").iterdir())
        self.assertTrue((run_dir / "artifacts" / "local-agent" / "selection.json").exists())
        self.assertTrue((run_dir / "artifacts" / "local-agent" / "command.txt").exists())
        selection = json.loads((run_dir / "artifacts" / "local-agent" / "selection.json").read_text())
        self.assertEqual(selection["provider"], "openai")
        self.assertEqual(selection["model"], "fake-codex")

    def test_local_agent_router_uses_claude_provider_fields(self) -> None:
        workspace = self.make_git_workspace(initial_value=1, mutated_value=9)
        (workspace / ".mutation.md").write_text("Raise the score if it is safe.\n")
        subprocess.run(["git", "add", ".mutation.md"], cwd=workspace, check=True)
        subprocess.run(
            ["git", "commit", "-m", "add mutation brief"],
        ...
        messages = run_once(self.paths, max_runs=1)
        self.assertEqual(len(messages), 1)
        self.assertEqual((workspace / "state.txt").read_text().strip(), "9")

        run_dir = next((self.paths.experiments / "local-claude" / "runs").iterdir())
        self.assertTrue((run_dir / "artifacts" / "local-agent" / "selection.json").exists())
        self.assertTrue((run_dir / "artifacts" / "local-agent" / "command.txt").exists())
        selection = json.loads((run_dir / "artifacts" / "local-agent" / "selection.json").read_text())
        self.assertEqual(selection["provider"], "claude")
        self.assertEqual(selection["model"], "claude-test")
